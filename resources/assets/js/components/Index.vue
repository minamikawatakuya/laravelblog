            ...
            <div class="col-md-8 col-md-offset-2">
                <div class="panel panel-default">
                    <div class="panel-body">
                        ...
                        <ul class="cate">
                            <li><a href="/">すべて({{blog_all_num}})</a></li>
                            <li v-for="category in categories" :key="category.id"><a v-on:click="selectCate(category.id)">{{category.name}}({{category.blog_num}})</a></li>
                            <li><a v-on:click="selectCate(0)">未選択({{blog_no_cate_num}})</a></li>
                        </ul>

                        <table>
                            <tr v-for="blog in blogs" :key="blog.id">
                                <td>
                                    <a v-on:click="toDetail(blog.id)">{{blog.title}} ({{blog.cate_name}})</a><br>
                                    {{blog.content_short}}
                                </td>
                            </tr>
                        </table>
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            axios.get('/api/blogNoCateNum')
            .then(response =>{
                this.blog_no_cate_num = response.data;
            });
